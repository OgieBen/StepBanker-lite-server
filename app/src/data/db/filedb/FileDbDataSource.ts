...
export class FileDbDataSource extends BaseDataSource {
    ...
    async bankUserSteps(bankedSteps: IBankedSteps): Promise<IBankedSteps> {
        try {
            this.checkDatabase();
            bankedSteps.id = DbUtils.getRandomInt();
            this.db.steps.push(bankedSteps);
            await FileDbUtils.saveDb(this.dbFilePath, this.db);
            return Promise.resolve(bankedSteps);
        } catch (e) {
            throw e;
        }
    }
    ...
    getAllBankedUserSteps(): Promise<IBankedSteps[]> {
        try {
            this.checkDatabase();
            return Promise.resolve(this.db.steps);
        } catch (e) {
            throw e;
        }
    }
    ...
    getLatestBankedStepByUserId(userId: string): Promise<IBankedSteps> {
        try {
            this.checkDatabase();
            const result = this.db.steps.find((steps: IBankedSteps) => steps.userId === userId);
            if (!result) {
                return Promise.reject('User Does not exist');
            }
            return Promise.resolve(result);
